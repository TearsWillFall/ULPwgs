...
build_clean_exec=function(
  ...
  .this.env=environment()
  append_env(to=.this.env,from=.env)
  .env$exec_code<- paste0(.env$exec_code," && rm ",paste(input,collapse=" "))
}
    ...
    if(!is.null(vars)){
        if(is.list(vars)){
          ...
          if(sum(check)>1){
            ...
            vars=unlist(vars[1])
            cat(orange("WARNING: No parallel argument was selected. No further parallelization outside the scope of this function will be applied","\n"))
          }else if(check==1){
