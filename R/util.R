...
add_sv_af_strelka_vcf=function(
  ...
  wait=FALSE,hold=""){
    ...
    vcf_dat$body=vcf_dat$body %>% dplyr::mutate(
      ...
      dplyr::select(-c(UALTPR,UREFPR,UALTSR,UREFSR))
    vcf_dat$body=vcf_dat$body %>% 
    dplyr::mutate(VALUE=ifelse(is.na(VALUE),"",VALUE))%>%
    tidyr::nest(FORMAT=FORMAT,VALUE=VALUE) %>% 
    dplyr::ungroup()%>% 
    ...
    add_pr_af_descriptor<-function(){
        list(
          Number="1",Type="Float",
          Description="\"Variant allelic frequency for supporting  reads\"")
    }

    add_sr_af_descriptor<-function(){
        list(
          Number="1",Type="Float",
          Description="\"Variant allelic frequency for supporting  split-reads\"")
    }
