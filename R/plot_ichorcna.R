...
plotCorrectionGenomeWide <- function(correctOutput, seqinfo = NULL, ...) {
  ...
  plotChrLines(as.vector(unique(GenomeInfoDb::seqnames(correctOutput))),coord$chrBkpt,yrange=r+c(-0.5,0.5))
...
plotChrLines <- function(chrs,chrBkpt,yrange){
  ...
  if (GenomeInfoDb::seqlevelsStyle(chrs)[1] != "NCBI"){
  	GenomeInfoDb::seqlevelsStyle(chrs) <- "NCBI"
  }
  chrs <- GenomeInfoDb::sortSeqlevels(unique(chrs))
